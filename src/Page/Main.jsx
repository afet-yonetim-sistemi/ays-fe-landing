...
import "../Styles.css";
import { FaGithub, FaDiscord, FaLanguage, FaAngleUp, FaAngleDown } from "react-icons/fa";

function MainPage() {
  const [language, setLanguage] = useState(
    localStorage.getItem("language") ? JSON.parse(localStorage.getItem("language")) : { name: "English", code: "en" }
  );
  ...
  return (
    <>
                ...
                <ul className="select-container">
                  {languageList.map((item) =>
                    item.code === language.code ? null : (
                      <li className="select-options" onClick={() => languageHandler(item)} key={item.code}>
                        {item.name}
                      </li>
                    )
                  )}
      ...
      <section className="intro-section">
        <div>
          <h1 className="intro-section-header">{paragraph.find((item) => item.language.code === language.code).header}</h1>
          <div
            className="intro-section-paragraph"
            dangerouslySetInnerHTML={paragraph.find((item) => item.language.code === language.code).paragraph}
          ></div>
        </div>
      </section>
      <section className="info-section">
        ...
        <div className="title-container ">
          <h1 className="info-section-header ">{paragraph.find((item) => item.language.code === language.code).header2}</h1>
          <div className="tech-container">
            ...
            <div className="tech-line">
              <svg xmlns="http://www.w3.org/2000/svg" width="7" height="256" viewBox="0 0 7 256" fill="none">
                <path
      ...
      <footer className="footer-container">
        <div className="footer-line">
          <svg xmlns="http://www.w3.org/2000/svg" width="1300" height="12" viewBox="0 0 1300 12" fill="none">
            <path d="M10 5L0 0.226497L0 11.7735L10 7V5ZM1290 7L1300 11.7735V0.226497L1290 5V7ZM9 7H1291V5H9V7Z" fill="white" />
          </svg>
          <div className="footer-content">
            <div className="footer-title">
              <p className="footer-title-paragraph">Afet Yönetim Sistemi | Copyright © 2023</p>
            </div>
